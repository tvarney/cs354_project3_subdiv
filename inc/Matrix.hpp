...
namespace cs354 {
    ...
    public:
        ...
        Matrix<T> translate(T x, T y, T z) {
            return (*this) * Matrix<T>::Matrix3D(1,0,0,x,0,1,0,y,0,0,1,z,
                                                 0,0,0,1);
            return Matrix<T>();
        }
        ...
        static Matrix<T> LookAt(Vector<T> eye) {
            return Matrix<T>::LookAt(eye, Vector<T>(0,0,0),
                                     Vector<T>(0,1,0));
        }
        static Matrix<T> LookAt(Vector3f eye, Vector3f up) {
            return Matrix<T>::LookAt(eye, Vector<T>(0,0,0), up);
        }
        static Matrix<T> LookAt(Vector3f eye, Vector3f center, Vector3f up) {
            Vector<T> forward = (center - eye).normalize();
            Vector<T> side = (forward * up).normalize();
            up = (side * forward);
            forward = -forward;
            Matrix<T> mat =
